...
var chladni = function() {
  ...
  function tick() {
    ...
    var freq = Math.PI * Math.sqrt(80) / canvas_width;
    var val = 100.0 * Math.sin(time * freq);
    pos_matrix[1][0] = val;
    pos_matrix[0][1] = val;
    pos_matrix[canvas_height - 1][1] = val;
    pos_matrix[canvas_height - 2][0] = val;
    pos_matrix[1][canvas_width - 1] = val;
    pos_matrix[0][canvas_width - 2] = val;
    pos_matrix[canvas_height - 2][canvas_width - 1] = val;
    pos_matrix[canvas_height - 1][canvas_width - 2] = val;
    time++;
    ...
    for (var r = 1; r < canvas_height - 1; r++) {
      for (var c = 1; c < canvas_width - 1; c++) {
        ...
        var vel_change = vel_matrix[r][c] / freq;
        var bright = 255 - 1000 * (pos_matrix[r][c] * pos_matrix[r][c] + vel_change * vel_change);
